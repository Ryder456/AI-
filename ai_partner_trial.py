...
with st.sidebar:
    ...
    # 手动输入模型地址和密钥
    st.subheader("web and key")
    # 无默认值，必须手动输入
    base_url = st.text_input("web", placeholder="请输入您所用大模型的Web")
    api_key = st.text_input("API_KEY", placeholder="请输入DeepSeek的API_KEY", type="password")
    ...
    if base_url and api_key:
        ...
        st.success("✅ 配置成功，可以开始聊天！")
    else:
        st.warning("⚠️ 请先输入 Web 和 API_KEY 才能使用AI")
